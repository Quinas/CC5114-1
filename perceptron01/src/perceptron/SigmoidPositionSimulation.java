...
public class SigmoidPositionSimulation extends AbstractSimulation {
  ...
  public void initNetwork(Double constant) {
    ...
    weights.add(1.0);
    weights.add(1.0);
    network = LogicGates.genericOneNeuronNetwork(weights, new SigmoidNeuron(0.0));
  ...
  public List<Double> calculateExpected(List<Double> input) {
    ...
    expected.add(2 * x > y ? 1.0 : 0.0); // Linear equation: y = 2x
    return expected;
  }

  @Override
  public boolean valid(List<Double> obtained, List<Double> expected) {
    if ((expected.get(0) == 1.0 && obtained.get(0) > 0.5)
        || (expected.get(0) == 0.0 && obtained.get(0) < 0.5)) {
      return true;
    } else {
      return false;
    }
